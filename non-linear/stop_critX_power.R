...
stop_critX_power <- function(cohort_num,cohort_res,cohort_dir = NULL,design.db=NULL, opt_setting=NULL,option=3,nsim=1000, CL_thetas=c(1,3,4),V_thetas=2,ci_corr=0,
  ...
  if(!is.null(cohort_res$est_result$cov_mat)){  
 ...
 for(i in 1:length(age_space)){
      ...
      for(j in 1:length(sub_group$AGE)){
        ...
        for(samp in 1:1000){
          ...
          SElcl[samp]=sd(params_df$LCL,na.rm=T)
          SELV[samp] =sd(params_df$LV,na.rm=T)
          }
#         CL_CI <- quantile(params_df$LCL, probs=c(0.025,0.975))
#         CL_CI <- CL_CI/gm_mean(params_df$LCL)
        
          SE_lcl <- median(SElcl)
          SE_LV <- median(SELV)
        
#         V_CI <- quantile(params_df$LV, probs=c(0.025,0.975))
#         V_CI <- V_CI/gm_mean(params_df$LV)
        ...
        CI_CL=c(exp(-qt((1-(alpha/2)),nsub-6)*SE_lcl),exp(qt((1-(alpha/2)),nsub-6)*SE_lcl))
        CI_V=c(exp(-qt((1-(alpha/2)),nsub-6)*SE_LV),exp(qt((1-(alpha/2)),nsub-6)*SE_LV))
        

        if(is.na(SElcl) | SElcl==Inf){
...
if(option==2){
  ...
  cov_lcl <- covmat[rownames(covmat) %in% lcl_thetas,colnames(covmat) %in% lcl_thetas]
  ...
  for(i in 1:length(age_space)){
    ...
    for(j in 1:length(sub_group$AGE)){
      ...
      if(ci_corr==1){ ##Bonferroni correction of CI
        ...
        CI_V=c(exp(-qt((1-(alpha/2)),nsub-6)*SE_V),exp(qt((1-(alpha/2)),nsub-6)*SE_V))

...
load("results_rep_1.Rdata")
cohort_res <- aod_res$cohort_4
...
nsim=10000
 CL_thetas=c(1,3)
 V_thetas=2
 sim_params=loglin_size_mat_scaling
  age_space = c(1,2,3,4,5,6)
  allow_next=T
  power=F)
  library(mvtnorm)
...
#   covmat <- as.matrix(covmat)
 ...
 for(i in 1:length(age_space)){
      ...
      for(j in 1:length(sub_group$AGE)){
        ...
        CI_CL_2=quartile(params_df$LCL,probs=c(0.025,0.975)
        CI_V_2=quartile(params_df$LV,probs=c(0.025,0.975)

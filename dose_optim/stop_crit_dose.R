...
stop_dose <- function(cohort_num,cohort_res,nsim=1000,samples=100,ci_corr=0,
                         alpha=0.05,sim_eff=PKPD.ff,sim_params=PKPD.fg,
                         power=F,lower_limit=0.8,higher_limit=1.2, use_FIM=F){
  library(mvtnorm)
  library(reshape)
  ...
  if(!is.null(cohort_res$est_result$cov_mat)  & fail_flag==F){  
    ...
    if (cohort_num>1){
     ...
     # true.db$parameters$param.pt.val$d[3,3] <-0.0625 
      ...
      true_res <- poped_optimize(true.db,opt_a = T, compute_inv=F) 
      
      #design_efficiency <-(det(evaluate.fim(true.db,fim.calc.type = 1))/det(evaluate.fim(cohort_res$opt_result$opt_output$poped.db,fim.calc.type = 1)))^(1/n_p) 
      
      design_efficiency <-(det(evaluate.fim(true.db,fim.calc.type = 1))/det(evaluate.fim(true_res$poped.db,fim.calc.type = 1)))^(1/n_p)
    ...
    return(list(stop_MBAOD,Eff_CIs,design_efficiency))

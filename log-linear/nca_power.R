...
##Set path to MBAOD_project directory
...
############################################################
####FOR ALL PLOTS AND RESULTS, RUN THE CODE FROM HERE#######
############################################################
...
#Scaling
load("Page_results/bridging_maturation_model_scaling_eval_run_dir_1/results_all.Rdata")
stop_crit_res <- 1:100
for(i in 1:100){
  stop_crit_res[i] <- results_all[[paste("iteration_",i,sep="")]]$stop_res$cohort_1[[1]]
  
}
power_scaling <- sum(stop_crit_res) #70
